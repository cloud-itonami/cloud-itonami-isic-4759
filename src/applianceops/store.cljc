(ns applianceops.store
  "SSoT for the ISIC-4759 'Retail sale of electrical household appliances,
  ...
  appliance/furniture/lighting specialty storefront: sales/inventory/
  ...
  registered vendors, and warranty/defect-dispute-concern flagging. It
  never directly finalizes a warranty-claim decision (approving, denying
  or settling a claim, authorizing a warranty refund or replacement) and
  it never directly finalizes a delivery/installation-safety clearance
  (certifying a gas/electrical hookup as safe, clearing an installation
  hazard as resolved) -- see `applianceops.governor`'s
  ...
  commit or escalate -- `applianceops.governor`'s
...
(defn demo-data
  ...
  []
  {:stores
   {"store-1" {:store-id "store-1" :name "Riverside Appliance & Furniture"
               :registered? true :verified? true}
    "store-2" {:store-id "store-2" :name "Sunset Boulevard Home Furnishings"
               :registered? true :verified? true}
    "store-3" {:store-id "store-3" :name "Downtown Pop-Up Lighting & Appliance Kiosk (in intake)"
               :registered? true :verified? false}}
   :vendors
   {"vendor-1" {:vendor-id "vendor-1" :name "Northgate Appliance & Furniture Wholesale"
                :registered? true :verified? true}
    "vendor-2" {:vendor-id "vendor-2" :name "Unverified Import Furnishings Broker Co."
                :registered? true :verified? false}}})
