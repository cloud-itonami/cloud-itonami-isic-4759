(ns applianceops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (ApplianceRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ApplianceRetailGovernor (:govern)
  ...
  human store operator/warranty coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [applianceops.advisor :as advisor]
            [applianceops.governor :as governor]
...
(defn build
  ...
  `applianceops.store/Store`).
  opts:
    :advisor      -- a `applianceops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ApplianceRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; ApplianceRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :store-id (:store-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :store-id (:store-id request) :by (:by approval)}]}
            {:disposition :hold
