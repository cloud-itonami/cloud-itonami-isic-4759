(ns applianceops.phase
  "Phase 0->3 staged rollout for the ISIC-4759 'Retail sale of electrical
  household appliances, furniture, lighting equipment and similar
  household articles in specialized stores' operations-coordination
  actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- sales-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds delivery/installation
                                       scheduling and supply-order
                                       proposals, still approval-gated.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                       `:log-sales-record`/
                                       `:schedule-delivery-operation`/
                                       `:coordinate-supply-order` may
                                       auto-commit. `:flag-warranty-
                                       concern` NEVER auto-commits, at any
                                       phase, and a high-cost `:coordinate-
                                       supply-order` still escalates even
                                       at phase 3 (the governor's own
                                       `high-stakes?` keeps it out of
                                       `:commit`).

  `:flag-warranty-concern` is deliberately ABSENT from every phase's
  `:auto` set, including phase 3 -- a permanent structural fact, not a
  rollout milestone still to come. Flagging a defect/warranty-dispute
  concern always needs a human to actually look at it; this actor never
  directly finalizes the claim decision itself.
  `applianceops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [applianceops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-warranty-concern` is a member of `write-ops`
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-sales-record :schedule-delivery-operation
                                                 :coordinate-supply-order}                                     :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-sales-record :schedule-delivery-operation :coordinate-supply-order}}})
...
(defn gate
  ...
  - `:flag-warranty-concern` is never auto-eligible at any phase, so it
...
(defn verdict->disposition
  "Map an ApplianceRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
