(ns applianceops.governor-contract-test
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-warranty-concern :store-id "store-1"
                                :patch {:concern "refrigerator compressor failed within warranty window" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "sofa set restock" :quantity 1 :estimated-cost 4600.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into warranty-claim/installation-safety-clearance-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-sales-record :store-id "store-1" :patch {:units-sold 42}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-sales-record :store-id "unknown" :patch {:units-sold 42}}
                     ctx)
